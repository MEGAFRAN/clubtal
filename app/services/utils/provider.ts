...
const getAll = async (apiUrl: string, endpoint: string) => {
  return await axios.get(`${apiUrl}/${endpoint}`).then(handleResponse).catch(handleError)
}

const getSingle = async (apiUrl: string, endpoint: string, id: AxiosRequestConfig<any>) => {
  return await axios.get(`${apiUrl}/${endpoint}/${id}`).then(handleResponse).catch(handleError)
}
...
const patch = async (apiUrl: string, endpoint: string, model: object) => {
  return await axios.patch(`${apiUrl}/${endpoint}`, model).then(handleResponse).catch(handleError)
}

const remove = async (apiUrl: string, endpoint: string, id: any) => {
  return await axios.delete(`${apiUrl}/${endpoint}/${id}`).then(handleResponse).catch(handleError)
}
